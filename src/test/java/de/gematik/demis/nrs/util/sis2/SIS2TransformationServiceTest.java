package de.gematik.demis.nrs.util.sis2;
...
class SIS2TransformationServiceTest {

  private final SIS2TransformationService legacy = new SIS2TransformationService();

  @MethodSource("conversionCandidates")
  @ParameterizedTest
  void doesHandleAllSpecialCharactersFromImport(final String candidate, final String expected) {
    final String actual = legacy.transformAsString(candidate);
  ...
  private static Stream<Arguments> conversionCandidates() {
    return Stream.of(
        ...
        Arguments.arguments(
            "04357 Leipzig Simón-Bolívar-Straße 99", "04357 LEIPZIG SIMON-BOLIVAR-STRASSE 99"),
        Arguments.arguments(
            "66802 Überherrn L'Hôpitaler Straße 2", "66802 UEBERHERRN L'HOPITALER STRASSE 2"),
        Arguments.arguments("25479 Ellerau Højerweg 9", "25479 ELLERAU HOEJERWEG 9"),
        Arguments.arguments(
